...
export default function Fridge() {
  const { t, lang, state, setFridge } = useApp()
  ...
  const matches = useMemo(() => {
    ...
    if (!q) return []
    return INGREDIENTS.filter((i) => !have.includes(i.id) && (i.name.fr.toLowerCase().includes(q) || i.name.ar.includes(query.trim()))).slice(0, 6)
  }, [query, have])
    ...
    <div className="screen scroll" style={{ paddingBottom: 24 }}>
        ...
        <div className="card" style={{ marginTop: 8, padding: 6 }}>
          {matches.map((i) => (
            <button key={i.id} onClick={() => add(i.id)} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 8px', width: '100%', textAlign: 'start' }}>
              <span style={{ fontSize: 18 }}>{i.emoji}</span><span style={{ fontWeight: 600 }}>{i.name[lang]}</span>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 14 }}>
        {QUICK.filter((id) => !have.includes(id)).map((id) => (
          <button key={id} onClick={() => add(id)} className="pill pill-amber">{ingredientMap[id].emoji} {ingredientMap[id].name[lang]}</button>
        ))}
      </div>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 16 }}>
          {have.map((id) => (
            <span key={id} className="pill pill-green">
              {ingredientMap[id]?.emoji} {ingredientMap[id]?.name[lang]}
